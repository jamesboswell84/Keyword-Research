...
if categories_csv is not None:
	...
	df1 = cat_data
	try:
		df2 = df1[df1.columns.drop(list(df1.filter(regex=r'Keywords|check_box|expand_less|Non\-Brands|.*\..*|^[0-9]*$')))]
	except:
		pass
	df2 = df1[df1.columns.drop(list(df1.filter(regex=r'Keywords|check_box|expand_less|Brand|Non\-Brands|.*\..*|^[0-9]*$')))]
